...
署名済みJSONファイルで接続される2つの半分でできています。
...
**市場に勝つ方法ではありません。** モデルの特徴量は市場由来の2列だけで、既定の混合比は
5%です。本リポジトリの合成データでは、holdoutのNLL改善は5.8のうち約0.015 natsです。
つまりほとんど差がありません。動的計画は**エッジがないことを前提**にしています。どの
価格帯も取った分より少なく払い戻すので、動的計画がやっているのは「いつ分散を受け入れるか」
の選択だけです。求めた値は `P(到達) ≤ R_max × 初期資金 / 目標` と照合され、これを超える
表はソルバが拒否します。

`make model` を実行して検証出力の `mean_final_bank` を見てください。毎回、初期資金を
下回ります。それがこの問題の正直な形です。
...
**対応環境はmacOSのみ**です。GoとPythonの部分は移植可能ですが、スケジューリング層は
launchdで、systemdやWindows向けの同等物は用意していません。
...
送信が起きるには、次が同時に成立している必要があります。ポリシーが送信を有効にしている、
ポリシーの日付と計画の日付が一致している、日付スコープの環境変数がポリシーと一致している、
運用者がバンドルのSHA-256を明示して当日を武装している、キルスイッチファイルが存在しない。
5段階あるのは意図的です。[docs/06-safety.md](docs/06-safety.md) を参照してください。
...
同じ形で真の確率が既知のデータを作る生成器です。自分のパネルを指定すれば、同じように動きます。

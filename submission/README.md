...
このディレクトリには、AI競馬予想マスターズ2026の期間中（2026年8月29日〜9月22日）に私が実際に動かしたスクリプトと、その結果を、当時の状態のまま入れています。運営への提出物として整理したものです。

リポジトリの `pykeiba/` とは役割が違います。`pykeiba/` はこれからも手を入れていく汎用のライブラリで、こちらは当時の記録です。他のコードから読み込んで使うことは想定しておらず、今後も修正しません。

## 各レースの投票方針

大会期間中、私は投票方針を何度か変えました。どの方針でどのレースに投票したかは次のとおりです。
...
8/30 からの V20 では、このとき方策表が示した行動の1つ（三連単936.5倍付近を3点、1点1,000pt）を固定し、それ以降は状態によらず各レースに同じ方針を適用することにしました。V20 の設定ファイル（`phase1/sanrentan3_policy.json`）には、その元になった方策と計画が記録されています。
...
どの方針でも、選んだ価格帯の中で具体的にどの組合せを買うかは、同じ方法で決めていました（`phase1/build_vote_plan.py:290` の `pick`。価格の近さ95%、Harville 確率の順位5%）。
...
実際のオッズは提供元に権利があるため入れていません。代わりに、合成データから同じ形式のオッズファイルを作って実行できるようにしました。
...
学習に使ったオッズは公開できないため、`phase2/optimizer.py` をそのまま実行することはできません。そこで、学習で最小化した関数そのものの値を公開することにしました。`nll_surface.npz` は、2つの重みを細かく変えたときの「レースあたりの平均 NLL」を、学習用と評価用のデータそれぞれについて記録したものです。これをなめらかにつないで最小値を探すと、記録されている重みが再現されます。

この値はレース全体の平均なので、個々の組合せのオッズは含まれておらず、ここからオッズを復元することはできません。ただし、この方法で確かめられるのは最適化の部分だけで、特徴量が正しく作られていたかまでは確かめられません。その点については、`model.json` に学習に使った155レース分の入力ファイルの SHA-256 を記録してあります。

## 結果について振り返って

- 9/20 のモデルによる評価用データでの NLL の改善は、5.8988 から 5.8957 へ、0.00305 とごくわずかでした。
...
- 確率は、候補の順位付けと金額の計算にだけ使いました。目標残高、組合せの絞り込み条件、投票を止める条件は、それとは別に私が決めたルールです。モデルが結果を予測して当てた、というものではありません。
...
差額 6,200 のうち 5,200 は、出走取消・競走除外による返還であることを確認しました。残る 1,000 は、まだ内訳を特定できていません。ごまかさずにそのまま `ledger/summary.json` に記録しています。

なお、私が以前つけていた台帳（3ファイル）には203レース・1,224,300pt が記録されていましたが、投票デーモンの記録（スナップショット）では204レース・1,227,300pt でした。差は 9/13 のレース `202606040404`（3,000pt）で、以前の台帳に記録漏れがありました。この台帳ではデーモンの記録を採用しています。このレースの払戻を `null` としている理由は、`ledger/summary.json` の `reconciliation_notes.commentary` に記載しています。

## 公開にあたって変更した点

プログラムの処理内容は変えていません。変更したのは次の点だけです。
...
認証情報、アカウント情報、学習に使った実際のオッズ、データの収集に使ったプログラムは入れていません。

## 公開について

このディレクトリの公開にあたっては、大会終了後に運営の皆さまへ確認し、ご了承をいただきました。ご対応いただき、ありがとうございました。

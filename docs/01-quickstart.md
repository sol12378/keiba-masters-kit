...
リポジトリを取得してから、模擬投票で実際に送信されるところまでを、順を追って説明します。所要時間は10分ほどで、そのほとんどは発走時刻を待つ時間です。
...
2つ目は `mean_final_bank`（最終残高の平均）です。初期資金の1,000,000を下回ります。この手法には市場を上回る優位性がないので、平均すると損をします。
...
データ形式と、とくに `captured_at`（オッズを取得した時刻）について気をつけていただきたいことは、[04-data-contract.md](04-data-contract.md) にまとめました。
...
詳しくは [03-launchd.md](03-launchd.md) をご覧ください。とくに、Mac がスリープすると投票できなくなる点について書いた節は、常駐させる前にぜひ読んでおいてください。
